...
class Kernel:
  ...
  def runner(self, agents = [], startTime = None, stopTime = None,
             num_simulations = 1, defaultComputationDelay = 0,
    ...
    for sim in range(num_simulations):
      ...
      pbar = tqdm(total=(self.stopTime-self.startTime)/pd.Timedelta(nanoseconds=1))
      ...
      while not self.messages.empty() and self.currentTime and (self.currentTime <= self.stopTime):
        ...
        pbar.update((self.currentTime-t_last)/pd.Timedelta(nanoseconds=1))
        t_last = self.currentTime
        msg_recipient, msg_type, msg = event
        try:
            if msg.body['event_idx'] != self.agents[0].refresh_count and msg.body['event_idx'] is not None:
                continue
        except:
            pass
        ...
        ttl_messages += 1
        # print(self.agents[0].refresh_count, self.currentTime)
