...
class OrderBook:
    ...
    def verification_of_price(self,order):
        diff = 0
        try:
            best_prices = [self.asks[0][0].limit_price, self.bids[0][0].limit_price]
            diff = abs(order.limit_price - best_prices[order.is_buy_order])
        except:
            pass
    ...
    def update_stream_history(self,order, currentTime, cancellation = False):
        self.event_history += [[(1-order.is_buy_order) * 2 + cancellation * 1, currentTime]]
        self.state_history += [self.compute_market_state()]


    def compute_market_state(self):
        if self.asks and self.bids:
            ask_volume = sum([o.quantity for o in self.asks[0]])
            bid_volume = sum([o.quantity for o in self.bids[0]])
            score = (bid_volume - ask_volume) / (ask_volume + bid_volume)
            if score > 0.4:
                return 2
            elif score < -0.4:
                return 0
    ...
    def handleLimitOrder(self, order, currentTime, market_order = False):
        ...
        self.history[0][order.order_id] = {'entry_time': self.owner.currentTime,
                                           ...
                                           'modifications': [],
                                           'cancellations': []}
        if not market_order and order.market_impact:
            if self.verification_of_price(order):
                self.update_stream_history(order, currentTime)
    ...
    def handleMarketOrder(self, order, currentTime):
        ...
        if order.market_impact:
            self.update_stream_history(order, currentTime)
        for lo in limit_orders.items():
            p, q = lo[0], lo[1]
            limit_order = LimitOrder(order.agent_id, order.time_placed, order.symbol, q, order.is_buy_order, p, market_impact=order.market_impact)
            self.handleLimitOrder(limit_order, currentTime, market_order = True)
    ...
    def cancelOrder(self, order, currentTime):
            ...
            if self.isEqualPrice(order, o[0]):
                ...
                for ci, co in enumerate(book[i]):
                    if order.order_id == co.order_id:
                        # Cancel this order.
                        within_price_levels = self.verification_of_price(order)
                        if within_price_levels and order.market_impact:
                            self.update_stream_history(order, currentTime, cancellation=True)
                        ...
                        if not book[i]:
                            del book[i]
                        if self.owner.book_freq is not None and within_price_levels:
                            row = {'QuoteTime': self.owner.currentTime}
                            for quote, volume in self.getInsideBids():
                                row[quote] = -volume
                                self.quotes_seen.add(quote)
    ...
    def cancelOrder_partial(self, order, partial_volume, currentTime):
        # Attempts to cancel (the remaining, unexecuted portion of) a trade in the order book.
        # By definition, this pretty much has to be a limit order.  If the order cannot be found
        # in the order book (probably because it was already fully executed), presently there is
        # no message back to the agent.  This should possibly change to some kind of failed
            ...
            if self.isEqualPrice(order, o[0]):
                ...
                for ci, co in enumerate(book[i]):
                    if order.order_id == co.order_id:
                        ...
                        try:
                            cancelled_order = book[i][ci]
                            cancelled_volume = partial_volume
                            cancelled_order.quantity = cancelled_order.quantity - cancelled_volume
                        except Exception:
                            cancelled_order = book[i].pop(ci)
                            cancelled_volume = cancelled_order.quantity
                            if not book[i]:
                                del book[i]
                        if self.owner.book_freq is not None and within_price_levels:
                            row = {'QuoteTime': self.owner.currentTime}
                        ...
                        self.owner.sendMessage(order.agent_id,
                                               Message({"msg": "ORDER_CANCELLED", "order": cancelled_order, "partial_volume": cancelled_volume}))
                        # We found the order and cancelled it, so stop looking.
                        self.last_update_ts = self.owner.currentTime
                        return
