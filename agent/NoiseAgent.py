...
class NoiseAgent(TradingAgent):
    ...
    def placeOrder(self,msg):
        ...
        if buy_indicator and ask:
            self.placeLimitOrder(self.symbol, self.size, buy_indicator, ask, tag = deepcopy(msg.body['refresh_count']))
        elif not buy_indicator and bid:
            self.placeLimitOrder(self.symbol, self.size, buy_indicator, bid, tag = deepcopy(msg.body['refresh_count']))
