...
class CT4LSTM_PPP(nn.Module):
    ...
    def read_iterative_sampling(self, batch, mkt_state, device = torch.device("cuda")):
        with torch.no_grad():
            ...
            x = torch.empty(size=(batch_length, samples, 4),device=device)
            x.exponential_(intens_upbound)
            u = torch.empty(size=(batch_length, samples, 4),device=device)
            u.uniform_()
            x_cumsum = x.cumsum(dim=1)
            index = torch.clip((x_cumsum / timestep + 1).long(),min = 0, max = samples-1)
            selected_intens = torch.take_along_dim(intens_at_samples, indices = index, dim = 1)
            a = selected_intens/intens_upbound > u
            a[:, -1, :] = torch.sum(a, dim=1)
            a[:, -1, :] = (a[:, -1, :] == 0)
            time_index_min = torch.argmax(a.long(), dim=1)
            pred_time, pred_type = torch.sort(torch.take_along_dim(x_cumsum, time_index_min[:,None,:],dim=1).squeeze(1),dim = -1)
...
class DroppedLinearV2(nn.Module):
    def __init__(self, input_dim, output_dim, bias=True):
        ...
        m1 = torch.tensor(output_dim*[1]+3*output_dim*[0]).unsqueeze(-1).repeat(1,input_dim).transpose(0,1)
        m2 = torch.vstack((m1,torch.tensor(output_dim*[0]+ output_dim*[1] +2*output_dim*[0]).unsqueeze(-1).repeat(1,input_dim).transpose(0,1)))
        m3 = torch.vstack((m2, torch.tensor(2 * output_dim * [0] + output_dim * [1] + output_dim * [0]).unsqueeze(-1).repeat(1,input_dim).transpose(0, 1)))
        self.m4 = torch.vstack((m3, torch.tensor(3 * output_dim * [0] + output_dim * [1]).unsqueeze(-1).repeat(1,input_dim).transpose(0, 1))).cuda()
