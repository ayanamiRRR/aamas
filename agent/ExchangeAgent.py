...
class ExchangeAgent(FinancialAgent):
  ...
  def __init__(self, id, name, type, mkt_open, mkt_close, symbols, book_freq='S', wide_book=False, pipeline_delay = 0,
  ...
  def receiveMessage(self, currentTime, msg):
    ...
    if msg.body['msg'] == "WHEN_MKT_OPEN":
      ...
      self.sendMessage(msg.body['sender'], Message({"msg": "QUERY_ORDER_STREAM", "symbol": symbol, "length": length,
                                                    ...
                                                    "bids": self.order_books[symbol].getInsideBids(1),
                                                    "asks": self.order_books[symbol].getInsideAsks(1),
                                                    }))
      ...
      if order.symbol not in self.order_books:
        ...
        self.order_books[order.symbol].handleLimitOrder(deepcopy(order),currentTime)
        if msg.body['event_idx'] is not None:
      ...
      if order.symbol not in self.order_books:
        ...
        self.order_books[order.symbol].handleMarketOrder(deepcopy(order), currentTime)
        if msg.body['event_idx'] is not None:
            self.publishOrderBookData()
      ...
      if order.symbol not in self.order_books:
        ...
        # Hand the order to the order book for processing.
        self.order_books[order.symbol].cancelOrder(deepcopy(order),currentTime)
        if msg.body['event_idx'] is not None:
            self.publishOrderBookData()

    elif msg.body['msg'] == "CANCEL_ORDER_PARTIAL":
      order = msg.body['order']
      partial_volume = msg.body['partial_volume']
      log_print("{} received CANCEL_ORDER: {}", self.name, order)
      if order.symbol not in self.order_books:
        log_print("Cancellation request discarded.  Unknown symbol: {}", order.symbol)
      else:
        # Hand the order to the order book for processing.
        self.order_books[order.symbol].cancelOrder_partial(deepcopy(order),partial_volume,currentTime)
        if msg.body['event_idx'] is not None:
            self.publishOrderBookData()
  ...
  def publishOrderBookData(self):
    ...
    for agent_id, params in self.subscription_dict.items():
      for symbol, values in params.items():
        ...
        if (freq == 0) or \
           ((orderbook_last_update > last_agent_update) and (orderbook_last_update - last_agent_update >= freq)):
          self.sendMessage(agent_id, Message({"msg": "MARKET_DATA",
                                              ...
                                              "volumes_last_seen": self.order_books[symbol].book_log[-1],
                                              "last_transaction": self.order_books[symbol].last_trade,
                                              "event_stream": self.order_books[symbol].event_history[-self.order_books[symbol].event_length:],
                                              "state_stream": self.order_books[symbol].state_history[-self.order_books[symbol].event_length:],
                                              "refresh_count": self.refresh_count,
