...
class ZeroIntelligenceAgent(TradingAgent):
    ...
    def __init__(self, id, name, type, symbol='IBM', starting_cash=100000, sigma_n=10,
                 r_bar=1000, kappa=0.05, sigma_s=10, q_max=10,
                 sigma_pv=10000, R_min=0, R_max=10, eta=1.0,
                 wake_up_freq=1, log_orders=False, random_state=None, seed = None, market_impact = False, folder_name = None):
        ...
        self.wake_up_freq = wake_up_freq  # mean arrival rate of ZI agents
    ...
    def kernelStopping(self):
        ...
        if self.__class__ == ZeroIntelligenceAgent:
            ...
            np.save('./log/{}/{}/event_ls_zi_{}_{}.npy'.format(self.folder_name,self.seed,self.id,self.market_impact),np.array(self.event_ls))
            np.save('./log/{}/{}/estimate_p_zi_{}_{}.npy'.format(self.folder_name,self.seed,self.id,self.market_impact), self.r_t_ls)
            np.save('./log/{}/{}/oracle_p_zi_{}.npy'.format(self.folder_name, self.seed, self.market_impact),self.oracle.r_t_ls)
    ...
    def updateEstimates(self):
        ...
        # to the shock variance. Bayesian updating
        self.r_t = (self.sigma_n / (self.sigma_n + sigma_tprime)) * r_tprime
        self.r_t += (sigma_tprime / (self.sigma_n + sigma_tprime)) * obs_t    #use obs to update prior
        ...
        self.r_t_ls.append((self.currentTime,self.r_t))
    ...
    def placeOrder(self, msg, market_impact):
        ...
        if bool(self.orders):
            ...
            self.event_ls.append([self.currentTime, 2 * (1 - buy), p, size, self.holdings['CASH'],
                          self.holdings['ABM']])
    ...
    def receiveMessage(self, currentTime, msg):
        ...
        if self.state == 'AWAITING_SPREAD':
            ...
            if msg.body['msg'] == 'QUERY_SPREAD':
                ...
                self.placeOrder(msg = msg,market_impact=self.market_impact)
                self.state = 'AWAITING_WAKEUP'
