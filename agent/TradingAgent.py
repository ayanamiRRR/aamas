...
class TradingAgent(FinancialAgent):
  ...
  def __init__(self, id, name, type, random_state=None, starting_cash=100000, log_orders=False, log_to_file=True):
    ...
    self.holdings = { 'CASH' : starting_cash , 'ABM' : 0}
    self.orders = {}
    ...
    self.order_dict_id_to_time = [{},{}]
    self.freq_first_called = True
  ...
  def receiveMessage (self, currentTime, msg):
    ...
    if msg.body['msg'] == "WHEN_MKT_OPEN":
      ...
      order = msg.body['order']
      try:
          cancelled_volume = msg.body['partial_volume']
      except Exception:
          cancelled_volume = order.quantity
      self.orderCancelled(order, cancelled_volume)
    ...
    if have_mkt_hours and not had_mkt_hours:
      ...
      self.wake_up_at_mkt_open_flag = True
  ...
  def placeLimitOrder (self, symbol, quantity, is_buy_order, limit_price, order_id=None, ignore_risk = True, tag = None, delay = 0, market_impact = True):

    order = LimitOrder(self.id, self.currentTime + pd.Timedelta(delay), symbol, quantity, is_buy_order, limit_price, order_id, market_impact = market_impact)
    if quantity > 0:
      ...
      self.sendMessage(self.exchangeID, Message({ "msg" : "LIMIT_ORDER", "sender": self.id,
                                                  "order" : order, "event_idx": tag }), delay = delay)
  ...
  def placeMarketOrder(self, symbol, quantity, is_buy_order, order_id=None, ignore_risk = True, tag=None, delay = 0, market_impact = True):
    """
      ...
      :return:
    """
    order = MarketOrder(self.id, self.currentTime+pd.Timedelta(delay), symbol, quantity, is_buy_order, order_id, market_impact = market_impact)
    if quantity > 0:
      ...
      if not ignore_risk:
        ...
        if (new_at_risk > at_risk) and (new_at_risk > self.starting_cash):
          ...
          return
      # self.orders[order.order_id] = deepcopy(order)
      self.sendMessage(self.exchangeID, Message({"msg" : "MARKET_ORDER", "sender": self.id, "order": order, "event_idx": tag if tag else None}), delay = delay)
      if self.log_orders: self.logEvent('ORDER_SUBMITTED', order.to_dict())
    else:
      log_print("TradingAgent ignored market order of quantity zero: {}", order)

  def cancelOrder(self, order, tag = None, delay = 0):
    ...
    if isinstance(order, LimitOrder):
      self.sendMessage(self.exchangeID, Message({"msg": "CANCEL_ORDER", "sender": self.id, "order": order, "event_idx": tag if tag else None}), delay = delay)
      # Log this activity.
  ...
  def cancelOrder_partial(self, order, partial_volume, tag = None, delay = 0):
    """Used by any Trading Agent subclass to cancel any order.  The order must currently
    appear in the agent's open orders list."""
    if isinstance(order, LimitOrder):
      self.sendMessage(self.exchangeID, Message({"msg": "CANCEL_ORDER_PARTIAL", "sender": self.id, "order": order, "partial_volume": partial_volume, "event_idx": tag if tag else None}), delay = delay)
      # Log this activity.
      if self.log_orders: self.logEvent('CANCEL_SUBMITTED', order.to_dict())
    else:
      log_print("order {} of type, {} cannot be cancelled", order, type(order))
  ...
  def orderExecuted (self, order):
    ...
    if order.order_id in self.orders:
      ...
      if order.quantity >= o.quantity:
          ...
          del self.order_dict_id_to_time[order.is_buy_order][order.order_id]
      else:
  ...
  def orderAccepted (self, order):
    ...
    self.orders[order.order_id] = deepcopy(order)
    self.order_dict_id_to_time[order.is_buy_order][order.order_id] = order.time_placed
  ...
  def orderCancelled (self, order, cancelled_volume):
    ...
    if order.order_id in self.orders:
      if cancelled_volume == self.orders[order.order_id].quantity:
          del self.orders[order.order_id]
          del self.order_dict_id_to_time[order.is_buy_order][order.order_id]
      else:
          self.orders[order.order_id].quantity = self.orders[order.order_id].quantity - cancelled_volume
    else:
  ...
  def getKnownBidAsk (self, symbol, best=True):
    if best:
      bid = self.known_bids[symbol][0][0] if self.known_bids[symbol] else self.known_asks[symbol][0][0] - 1
      ask = self.known_asks[symbol][0][0] if self.known_asks[symbol] else self.known_bids[symbol][0][0] + 1
